...
Status ADD_SUBFUNC(CrossM *M, CrossM *N, CrossM *Q, int f)
{
        ...
        while(p1 && p2)
        {
            if(p1->j == p2->j)
            {
                ...
                if(x)
                {
                    ...
                    {
                        for(p3 = Q->rhead[i]; p3->right && p3->j < p->j; p3 = p3->right);
                        p->right = p3->right;
                        p3->right = p;
                    }
                }
            ...
            else if(p1->j < p2->j)
            {
                ...
                {
                    for(p3 = Q->rhead[i]; p3->right && p3->j < p1->j; p3 = p3->right);
                    p->right = p3->right;
                    p3->right = p;
                }
                p1 = p1->
...
Status Display(CrossM *M)
{
    ...
    int i, j, t;
    t = 1;
    printf("Transpose ,The result is :\n");
    for(i = 1; i<=M->mu; i++)
    {
        //从每一行头指针开始遍历
        OLNode *p = M->rhead[i];
